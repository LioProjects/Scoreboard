...
export class GameService {
  ...
  private gameModeSubject = new BehaviorSubject<GameModeState>(
  ...
  setNextPlayerTurn() {
    ...
    const currentPlayerTurnIndex = players.findIndex(
  ...
  addGamePoint(newGamePoint: PlayerGamePoint) {
    this.playerGamePointHistoryList.next([
      ...this.playerGamePointHistoryList.value,
      newGamePoint,
    ]);
  ...
  recordPlayerScore(player: Player, pointValue: number) {
    const moneyballInPlay = this.moneyballEnabled
      ? this.popMoneyball()
      : undefined;
    const newGamePoint = new PlayerGamePoint(
      player,
      pointValue,
      moneyballInPlay
    );
    this.gameModeSubject
      .getValue()
      .recordPlayerScore(newGamePoint, this.currentGameSubject.getValue());
  }
  ...
  undo() {
    ...
    this.currentGameSubject.next(
      this.gameHistoryList[this.gameHistoryList.length - 1]
    );
    let undonePlayerGamePoint =
      this.playerGamePointHistoryList.value[
        this.playerGamePointHistoryList.value.length - 1
      ];
    if (undonePlayerGamePoint?.moneyball) {
      this.moneyBallQueueSubject.next([
        ...this.moneyBallQueueSubject.getValue(),
        undonePlayerGamePoint.moneyball,
      ]);
    } else if (undonePlayerGamePoint?.player) {
      this.currentPlayerTurnSubject.next(undonePlayerGamePoint.player);
    }
    this.playerGamePointHistoryList.next([
      ...this.playerGamePointHistoryList.value.slice(0, -1),
    ]);
  }
  ...
  private calculateStatistics(newGamePoint: PlayerGamePoint) {
    ...
    if (
      !currentGame.playerStatistics.find(
        (statistic) => statistic.playerId === newGamePoint.player._id
      )
    ) {
    ...
    currentGame.playerStatistics.forEach((playerStatistic) => {
      ...
      if (playerStatistic.playerId !== newGamePoint.player._id) {
        newPlayerNettoScore =
          playerStatistic.nettoScore +
          (newGamePoint.moneyball?.multiplierForOpponent ?? 0) *
            newGamePoint.pointValue *
            newGamePoint.multiplier;
        newPlayerAdditiveScore = [...playerStatistic.additiveScore];
        newPlayerAdditiveScore[playerStatistic.additiveScore.length - 1] =
          newPlayerNettoScore;
        newPlayerSufferedMalus =
          playerStatistic.sufferedMalus +
          (newGamePoint.moneyball?.multiplierForOpponent ?? 0) *
            newGamePoint.pointValue;
      } else {
        newPlayerNettoScore =
          playerStatistic.nettoScore +
          (newGamePoint.moneyball?.multiplierForShooter ?? 1) *
            newGamePoint.pointValue *
            newGamePoint.multiplier;
        newPlayerBruttoScore =
          playerStatistic.bruttoScore + newGamePoint.pointValue;
